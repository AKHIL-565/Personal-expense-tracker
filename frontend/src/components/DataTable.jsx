        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse transaction-table">
                    <thead>
                        <tr className="bg-surface-highlight text-secondary text-xs uppercase tracking-wider">
                    ...
                    <tbody className="divide-y divide-border-color">
                        {transactions.map((t, index) => (
                            <tr key={t._id || t.id} style={{ backgroundColor: index % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)' }}>
                                <td className="p-sm text-sm border-r border-border-color/10 date-cell">{format(new Date(t.date), 'dd/MM/yyyy')}</td>
                                <td className={`p-sm text-sm font-bold border-r border-border-color/10 ${t.type === 'Income' ? 'text-success' :
                                    t.type === 'Expense' ? 'text-danger' : 'text-primary'
                                    }`}>
                                    {t.type}
                                </td>
                                <td className="p-sm text-sm border-r border-border-color/10">{t.mode}</td>
                                <td className="p-sm text-sm border-r border-border-color/10">{t.category}</td>
                                <td className={`p-sm text-sm text-right font-mono font-bold border-border-color/10 ${isDeleteMode ? 'border-r' : ''}`}>₹{t.amount.toFixed(2)}</td>
                                {isDeleteMode && (
                                    <td className="p-sm text-sm text-center">
                                        <button
